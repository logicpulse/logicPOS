...
{
    internal class DialogConfigurationPlaceTable : EditDialog
    {
        public DialogConfigurationPlaceTable(Window parentWindow, XpoGridView pTreeView, DialogFlags pFlags, DialogMode pDialogMode, Entity pXPGuidObject)
        ...
        private void InitUI()
        {
            try
            {
                ...
                //ConfigurationPlace
                XPOComboBox xpoComboBoxPlace = new XPOComboBox(Entity.Session, typeof(pos_configurationplace), (Entity as pos_configurationplacetable).Place, "Designation", null);
                BOWidgetBox boxPlace = new BOWidgetBox(CultureResources.GetResourceByLanguage(LogicPOS.Settings.CultureSettings.CurrentCultureName, "global_place"), xpoComboBoxPlace);
                vboxTab1.PackStart(boxPlace, false, false, 0);
                InputFields.Add(new GenericCRUDWidgetXPO(boxPlace, Entity, "Place", LogicPOS.Utility.RegexUtils.RegexGuid, true));
                ...
                if (Entity != null && (Entity as pos_configurationplacetable).Oid == POSSettings.XpoOidConfigurationPlaceTableDefaultOpenTable) checkButtonDisabled.Sensitive = false;
