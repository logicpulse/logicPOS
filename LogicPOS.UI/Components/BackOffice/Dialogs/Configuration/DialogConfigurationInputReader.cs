...
{
    internal class DialogConfigurationInputReader : EditDialog
    {
        public DialogConfigurationInputReader(Window parentWindow, XpoGridView pTreeView, DialogFlags pFlags, DialogMode pDialogMode, Entity pXPGuidObject)
            : base(parentWindow, pTreeView, pFlags, pDialogMode, pXPGuidObject)
        {
            this.Title = logicpos.Utils.GetWindowTitle(CultureResources.GetResourceByLanguage(LogicPOS.Settings.CultureSettings.CurrentCultureName, "window_title_edit_configurationinputreader"));
        ...
        private void InitUI()
        {
            try
            {
                ...
                BOWidgetBox boxReaderSizes = new BOWidgetBox(CultureResources.GetResourceByLanguage(LogicPOS.Settings.CultureSettings.CurrentCultureName, "global_input_reader_card_reader_sizes"), entryReaderSizes);
                vboxTab1.PackStart(boxReaderSizes, false, false, 0);
                InputFields.Add(new GenericCRUDWidgetXPO(boxReaderSizes, Entity, "ReaderSizes", LogicPOS.Utility.RegexUtils.RegexIntegerSplittedByComma, true));
