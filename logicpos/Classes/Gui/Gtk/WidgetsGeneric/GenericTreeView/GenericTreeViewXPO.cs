    ...
    internal class GenericTreeViewXPO : GenericTreeView<XPCollection, XPGuidObject>
    {
        ...
        )
        {
            ...
            if (pXpoCollection.Sorting.Count == 0)
            {
                try
                {
                    pXpoCollection.Sorting = SharedUtils.GetXPCollectionDefaultSortingCollection();
                }
                catch (Exception ex)
                {
                ...
                for (int i = 0; i < _columnProperties.Count; i++)
                {
                    ...
                    try
                    {
                          ...
                          dataRow.GetMemberValue(_columnProperties[i].Name).GetType().BaseType == typeof(XPGuidObject))
                        {
                            ...
                            if (value != null)
                            {
                                if (GetXPGuidObjectChildValue(dataRow.GetMemberValue(_columnProperties[i].Name), fieldName, _columnProperties[i].ChildName).Equals("False") || GetXPGuidObjectChildValue(dataRow.GetMemberValue(_columnProperties[i].Name), fieldName, _columnProperties[i].ChildName).Equals("True"))
                                {
                                    ...
                                    columnValues[i] = (booleanValue) ? resources.CustomResources.GetCustomResource(LogicPOS.Settings.GeneralSettings.Settings["customCultureResourceDefinition"], "global_treeview_true") : resources.CustomResources.GetCustomResource(LogicPOS.Settings.GeneralSettings.Settings["customCultureResourceDefinition"], "global_treeview_false");
                                }
                                else
                                {
                        ...
                        if (_columnProperties[i].ResourceString == true && columnValues[i] != null)
                        {
                            ...
                            bool checkIfResourceStringExist = (resources.CustomResources.GetCustomResource(LogicPOS.Settings.GeneralSettings.Settings["customCultureResourceDefinition"], columnValues[i].ToString()) != null);
                            ...
                            {
                                columnValues[i] = resources.CustomResources.GetCustomResource(LogicPOS.Settings.GeneralSettings.Settings["customCultureResourceDefinition"], columnValues[i].ToString());
                            }
