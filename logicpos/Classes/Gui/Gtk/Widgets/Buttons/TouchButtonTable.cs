    ...
    public class TouchButtonTable : TouchButtonText
    {
        ...
        public void InitObject(string pName, Color pColor, string pLabelText, string pFont, TableStatus pTableStatus, decimal pTotal, DateTime pDateOpen, DateTime pDateClosed)
        {
            ...
            switch (TableStatus)
            {
                ...
                case TableStatus.Open:
                    ...
                    if (pDateOpen != null) labelDateTableOpenOrClosed.Text = string.Format(CultureResources.GetLanguageResource(LogicPOS.Settings.GeneralSettings.Settings.GetCultureName(), "pos_button_label_table_open_at"), pDateOpen.ToString(LogicPOS.Settings.CultureSettings.DateTimeFormatHour));
                    SetBackgroundColor(_colorPosTablePadTableTableStatusOpenButtonBackground, _eventBoxTotalOrStatus);
                    break;
                case TableStatus.Reserved:
                    _labelTotalOrStatus.Text = CultureResources.GetLanguageResource(LogicPOS.Settings.GeneralSettings.Settings.GetCultureName(), "global_reserved_table");
        ...
        public void ChangeTableStatus(Guid pTableOid, TableStatus pTableStatus)
        {
            ...
            if (pTableStatus == TableStatus.Reserved)
            {
                ...
                SharedUtils.Audit("TABLE_RESERVED", string.Format(CultureResources.GetLanguageResource(LogicPOS.Settings.GeneralSettings.Settings.GetCultureName(), "audit_message_table_reserved"), xTable.Designation));
            }
            else
            {
                ...
                SharedUtils.Audit("TABLE_UNRESERVED", string.Format(CultureResources.GetLanguageResource(LogicPOS.Settings.GeneralSettings.Settings.GetCultureName(), "audit_message_table_unreserved"), xTable.Designation));
            }
            //_logger.Debug(string.Format("1 pTableStatus: [{0}] [{1}]", xTable.Designation, pTableStatus));
            //Update Status State  
            TableStatus = pTableStatus;
